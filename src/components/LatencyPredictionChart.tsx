...
import { TrendingUp, Sparkles, Calendar, Info, Clock, AlertTriangle } from 'lucide-react';
...
const synodicCyclePoints = [
  { month: 'M0', monthNum: 0, delayMin: 3.0, distanceM: 54.6, x: 20, y: 190 },
  { month: 'M2', monthNum: 2, delayMin: 5.2, distanceM: 93.6, x: 60, y: 170 },
  { month: 'M4', monthNum: 4, delayMin: 9.1, distanceM: 163.8, x: 100, y: 140 },
  { month: 'M8', monthNum: 8, delayMin: 16.0, distanceM: 288.0, x: 180, y: 80 },
  { month: 'M12', monthNum: 12, delayMin: 22.0, distanceM: 396.0, x: 260, y: 30 },
  { month: 'M13', monthNum: 13, delayMin: 22.3, distanceM: 401.0, x: 280, y: 25, isConjunction: true },
  { month: 'M16', monthNum: 16, delayMin: 18.0, distanceM: 324.0, x: 340, y: 65 },
  { month: 'M20', monthNum: 20, delayMin: 11.0, distanceM: 198.0, x: 420, y: 125 },
  { month: 'M24', monthNum: 24, delayMin: 5.1, distanceM: 91.8, x: 500, y: 172 },
  { month: 'M26', monthNum: 26, delayMin: 3.0, distanceM: 54.6, x: 540, y: 190 },
];
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800/80 pb-4">
        <div>
          <span className="text-xs font-mono text-earth-400 uppercase tracking-wider font-semibold flex items-center gap-1.5">
            <TrendingUp className="w-4 h-4" /> Predictive Latency Engine
          </span>
          <h3 className="text-lg font-bold font-heading text-white mt-1">
            26-Month Synodic Latency Curve & Orbital Forecast
          </h3>
        </div>
        <div className="flex items-center gap-2 font-mono text-xs text-slate-400 bg-space-950 px-3 py-1.5 rounded-lg border border-slate-800 self-start sm:self-auto">
          ...
          <span>Cycle: ~780 Earth Days</span>
        </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        
        {/* Native SVG High-Reliability Chart */}
        <div className="lg:col-span-2 bg-space-950 p-4 rounded-xl border border-slate-800/80 flex flex-col justify-between relative overflow-hidden">
          <div className="flex items-center justify-between text-xs font-mono text-slate-400 mb-2">
            <span>Signal Delay Over Synodic Cycle (Minutes)</span>
            <span className="text-earth-400 font-semibold">Min: 3m | Peak: ~22.3m</span>
          </div>
          ...
          <div className="w-full relative py-2">
            <svg viewBox="0 0 560 230" className="w-full h-auto overflow-visible">
              ...
              {/* Solar Conjunction Vertical Line (M13) */}
          ...
          <div className="flex items-center justify-between text-[11px] font-mono text-slate-400 border-t border-slate-800/80 pt-2 mt-2">
            <span className="flex items-center gap-1"><span className="w-2.5 h-2.5 rounded-full bg-mars-500"></span> Synodic Latency Path</span>
            <span className="flex items-center gap-1"><span className="w-2.5 h-2.5 rounded-full bg-red-500"></span> Solar Conjunction Peak</span>
          </div>
        </div>
        ...
        <div className="bg-space-950 p-5 rounded-xl border border-slate-800 flex flex-col justify-between space-y-4">
          ...
          <div className="bg-space-900/90 border border-slate-800 p-3.5 rounded-xl space-y-2 font-mono text-xs">
            ...
            <div className="flex justify-between items-center">
              ...
              <span className={`font-semibold ${telemetry.synodicMonth <= 13 ? 'text-amber-400' : 'text-emerald-400'}`}>
                {telemetry.synodicMonth <= 13 ? '↑ Increasing Delay' : '↓ Decreasing Delay'}
              </span>
            </div>
          </div>
          ...
          <div className="bg-slate-900/60 border-l-4 border-mars-500 p-3 rounded-r-xl text-xs text-slate-300 italic font-mono leading-relaxed">
            "{telemetry.isConjunction 
              ? 'Communication blackout in effect. Pre-loaded autonomous rule set active on spacecraft.'
              : telemetry.synodicMonth > 10 && telemetry.synodicMonth < 16
              ? 'High latency orbit phase. Solar conjunction approaching in next synodic window. Recommend pre-loading command sequences now.'
              : 'Communication delay is within normal limits. Prioritize high-resolution imaging and scientific data transmission during current window.'}"
          </div>

          <div className="flex items-center gap-2 text-[11px] text-slate-400 font-mono">
            <Info className="w-3.5 h-3.5 text-earth-400 shrink-0" />
            <span>AI model turns raw physical delay into actionable mission plans.</span>
          </div>
